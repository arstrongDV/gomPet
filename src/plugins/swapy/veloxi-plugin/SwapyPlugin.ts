...
export type SwapData = RequireOnlyOne<SwapEventDataData, 'map' | 'array' | 'object'>;
...
function createEventData(eventMap: SwapEventMap): SwapEventDataData {
  ...
  return {
    ...
    object: Array.from(eventMap).reduce<SwapEventObject>((result, [slot, item]) => {
      result[slot] = item;
      return result;
    }, {})
  };
...
function createEventDataFromAny(data: SwapData): SwapEventDataData {
  if (data.map) {
    ...
    return {
      ...
      object: Array.from(data.map).reduce<SwapEventObject>((result, [slot, item]) => {
        result[slot] = item;
        return result;
      }, {})
    };
...
export const SwapyPlugin: PluginFactory<SwapyConfig, SwapyPluginApi> = (context) => {
  ...
  function setupSlot(slot: View) {
    ...
    slotItemMap.set(slot.element.dataset.swapySlot!, item ? item.element.dataset.swapyItem! : null);
  }
  ...
  function updateDraggingPosition() {
    ...
    if (!offsetX || !offsetY) {
      ...
      offsetX = draggingEvent.pointerX - draggingItem.position.x + draggingItemScrollOffset.x;
      offsetY = draggingEvent.pointerY - draggingItem.position.y + draggingItemScrollOffset.y;
    }
