...
function validate(root: HTMLElement): boolean {
  ...
  slotElements.forEach((slot) => {
    ...
    if (slotChildren.length > 1) {
      console.error('slot:', `"${slotId}"`, 'cannot contain more than one element');
      isValid = false;
    }
    const slotChild = slotChildren[0] as HTMLElement;
    if (slotChild && (!slotChild.dataset.swapyItem || slotChild.dataset.swapyItem.length === 0)) {
      console.error('slot:', `"${slotId}"`, 'does not contain an element with item id using data-swapy-item');
      isValid = false;
    }
  });
  return isValid;
...
function addVeloxiDataAttributes(root: HTMLElement, config = {} as Config): string {
  ...
  const slots = Array.from(root.querySelectorAll('[data-swapy-slot]')) as HTMLElement[];
  slots.forEach((slot) => {
    slot.dataset.velView = 'slot';
  });

  const items = Array.from(root.querySelectorAll('[data-swapy-item]')) as HTMLElement[];
  items.forEach((item) => {
  ...
  const textElements = Array.from(root.querySelectorAll('[data-swapy-text]')) as HTMLElement[];
  textElements.forEach((el) => {
    el.dataset.velLayoutPosition = '';
  });

  const excludedElements = Array.from(root.querySelectorAll('[data-swapy-exclude]')) as HTMLElement[];
...
function resyncItems(root: HTMLElement): boolean {
  const slots = Array.from(root.querySelectorAll('[data-swapy-slot]:not([data-vel-view])')) as HTMLElement[];
  slots.forEach((slot) => {
    slot.dataset.velView = 'slot';
  });
  const items = Array.from(root.querySelectorAll('[data-swapy-item]:not([data-vel-view]')) as HTMLElement[];
  items.forEach((item) => {
    item.dataset.velView = 'item';
    item.dataset.velLayoutId = item.dataset.swapyItem;
    const handle = item.querySelector('[data-swapy-handle]') as HTMLElement;
    ...
    const textElements = Array.from(item.querySelectorAll('[data-swapy-text]')) as HTMLElement[];
    textElements.forEach((el) => {
      el.dataset.velLayoutPosition = '';
    });

    const excludedElements = Array.from(item.querySelectorAll('[data-swapy-exclude]')) as HTMLElement[];
    excludedElements.forEach((el) => {
      el.dataset.velIgnore = '';
    });
  });
...
function createSwapy(root: Element | null, userConfig: Partial<Config> = {} as Partial<Config>): SwapyApi {
  if (!root) {
    throw new Error('Cannot create a Swapy instance because the element you provided does not exist on the page!');
  }
...
class SwapyInstance {
  ...
  constructor(rootEl: HTMLElement, pluginKey: string, config: Partial<Config>) {
    ...
    this._veloxiApp.onPluginEvent(
      ...
      SwapEvent,
      (event) => {
        if (this._previousMap && mapsAreEqual(this._previousMap, event.data.map)) {
          return;
        }
        if (!config.manualSwap) {
  ...
  private _createSlotElMap() {
    return (Array.from(this._rootEl.querySelectorAll('[data-swapy-slot]')) as HTMLElement[]).reduce((map, el) => {
  ...
  private _createItemElMap() {
    return (Array.from(this._rootEl.querySelectorAll('[data-swapy-item]')) as HTMLElement[]).reduce((map, el) => {
